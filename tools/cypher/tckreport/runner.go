...
func matchExpectedError(expectation string, actual error) (matched, supported bool) {
	...
	patterns, exists := map[string][]string{
		...
		"DeleteConnectedNode":             {"cannot delete node"},
		"NegativeIntegerArgument":         {"non-negative integer"},
		"InvalidArgumentType":             {"got ", "cannot index"},
		"InvalidArgumentValue":            {"invalid value type", "labels expects a node", "type expects a relationship"},
		"NumberOutOfRange":                {"out of range", "exceeds", "must be between", "step must be non-zero"},
		...
		"InvalidAggregation":              {"does not allow aggregate", "not allowed inside", "cannot introduce aggregation"},
		"InvalidDelete":                   {"delete expects"},
		"InvalidParameterUse":             {"parameter cannot be used directly"},
		"UnexpectedSyntax":                {"pattern expression is not allowed in this context"},
		"InvalidClauseComposition":        {"cannot mix union", "exists subquery cannot contain updating clauses"},
		"NoVariablesInScope":              {"requires at least one variable"},
		"NestedAggregation":               {"cannot be nested"},
		"NonConstantExpression":           {"non-constant"},
		...
		"RelationshipUniquenessViolation": {"relationship variable", "cannot be reused"},
		"NoExpressionAlias":               {"must be aliased"},
	}[code]
	if !exists {
		return false, false
	}
